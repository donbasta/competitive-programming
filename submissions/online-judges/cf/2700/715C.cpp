...
void solve() {
    ...
    vector<int> v1(n), v2(n), dep(n);  // v1 naik, v2 turun;
    ...
    const function<int(int, int)> naik = [&](int a, int b) -> int {
        int ret = (v1[a] - v1[b]) % mod;
        ret = (1ll * ret * ip10[dep[b]]) % mod;
        if (ret < 0) ret += mod;
        return ret;
    };
    const function<int(int, int)> turun = [&](int a, int b) -> int {
        int ret = (1ll * v2[b] - 1ll * v2[a] * p10[dep[b] - dep[a]]) % mod;
        if (ret < 0) ret += mod;
        return ret;
    };
    const function<int(int, int)> num = [&](int a, int b) -> int {
        int l = lca(a, b);
    ...
    ll ans = 0;
    vector<unordered_map<int, int>> cnt_naik(n), cnt_turun(n);
