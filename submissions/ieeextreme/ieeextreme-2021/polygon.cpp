...
bool is_on_line(const point& a, const point& b, const point& c) {
    point ab = b - a;
    point ac = c - a;
    ll u = ab.y * ac.z - ab.z * ac.y;
    ll v = ab.z * ac.x - ab.x * ac.z;
    ll w = ab.x * ac.y - ab.y * ac.x;
    return point(u, v, w) == point();
}
...
ll calc_plane_with_boundary(const point& a, const point& b, const point& c) {
    return calc_plane(a, b, c) + calc_segment(a, b) + calc_segment(b, c) + calc_segment(a, c) + 3;
}
...
void solve() {
    ...
    // vector<ll> dir;
    // while (j < n - 1) {
    //     bool lul = is_on_line(pts[0], pts[j], pts[j + 1]);
    //     if (lul) {
    //         j++;
    //     } else {
    //         dir = find_plane(pts[0], pts[j], pts[j + 1]);
    //         break;
    //     }
    // }
