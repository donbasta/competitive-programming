...
BigInt egcd(BigInt a, BigInt b, BigInt &x, BigInt &y) {
    ...
    BigInt x1, y1;
    BigInt d = egcd(b, a % b, x1, y1);
    x = y1;
    y = x1 - y1 * (a / b);
    return d;
}

BigInt ceilz(BigInt a, BigInt b) {
    if (a % b == 0) {
        return a / b;
    }
    if (b < 0) {
...
void el_psy_congroo() {
    ...
    BigInt b = 1;
    if (idx != -1) {
        b = fpow(BigInt(10), len - 1 - idx);
        s = s.substr(0, idx) + s.substr(idx + 1, len - 1 - idx);
    }
    BigInt a = BigInt(s);
    ...
    BigInt fp = gcd(a, 90);
    BigInt a1 = a / fp;
    BigInt l = BigInt(90) / fp;
    BigInt n = 0;
    if (a1 % 2 == 1) {
        n = b * l;
    } else {
        BigInt xx, yy;
        BigInt dd = egcd(a1, l * b, xx, yy);
        BigInt kpk = lcm(l * b, a1);

        BigInt kk = ceilz((BigInt(1) - xx) * a1, kpk);
        BigInt nn = xx + (kk * kpk) / a1;

        BigInt kkk = ceilz((BigInt(1) + xx) * a1, kpk);
        BigInt nnn = -xx + (kkk * kpk) / a1;
