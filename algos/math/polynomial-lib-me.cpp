...
void normalize(poly& pol) {while ((int)pol.size() && pol.back() == 0) pol.pop_back();}
poly reverse(poly pol) {reverse(pol.begin(), pol.end()); return pol;}
poly shift(poly pol, int x) {p.insert(p.begin(), x, 0); return p;}
poly cut(poly pol, int x) {p.resize(x); return p;}
...
poly operator-(poly a, const poly& b) { return a -= b; }
poly operator-(poly a) { for(auto& i: a) i *= -1; return a; }
